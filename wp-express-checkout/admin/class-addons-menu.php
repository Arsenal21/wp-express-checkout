...
class Addons_Admin_Menu {
	...
	public function render_addons_menu_page() {
		...
		echo '<div class="wrap">';
		echo '<h1>' . esc_html__( 'WP Express Checkout Extensions', 'wp-express-checkout' ) . '</h1>';
		...
		$addon_1 = array(
			"name"		 => esc_html__( "Subscription Payments Addon", 'wp-express-checkout' ),
			"thumbnail"	 => WPEC_PLUGIN_URL . "/admin/includes/images/wpec-subscriptions.png",
			"description"	 => esc_html__( "This addon allows you to charge subscriptions or recurring payments.", 'wp-express-checkout' ),
			"page_url"	 => "https://wp-express-checkout.com/subscription-payments-addon-wp-express-checkout/",
		);
		array_push( $addons_data, $addon_1 );

		$addon_2 = array(
			"name"		 => esc_html__( "Custom Fields Addon", 'wp-express-checkout' ),
			"thumbnail"	 => WPEC_PLUGIN_URL . "/admin/includes/images/wpec-custom-fields.png",
			"description"	 => esc_html__( "Collect information from customers before they checkout within the payment window.", 'wp-express-checkout' ),
			"page_url"	 => "https://wp-express-checkout.com/custom-fields-addon/",
		);
		array_push( $addons_data, $addon_2 );

		$addon_3 = array(
			"name"		 => esc_html__( "Reset Settings and Data", 'wp-express-checkout' ),
			"thumbnail"	 => WPEC_PLUGIN_URL . "/admin/includes/images/wpec-reset-settings.png",
			"description"	 => esc_html__( "Allows you to reset the entire settings configuration and data of the WP Express Checkout Plugin.", 'wp-express-checkout' ),
			"page_url"	 => "https://wp-express-checkout.com/wp-express-checkout-reset-settings-and-data/",
		);
		array_push( $addons_data, $addon_3 );

		$addon_4 = array(
			"name"		 => esc_html__( "WP eMember Integration", 'wp-express-checkout' ),
			"thumbnail"	 => WPEC_PLUGIN_URL . "/admin/includes/images/wpec-wp-emember-integration.png",
			"description"	 => esc_html__( "Integrate with the WP eMember plugin to accept membership payments.", 'wp-express-checkout' ),
		...
		$addon_5 = array(
			"name"		 => esc_html__( "Simple Membership Integration", 'wp-express-checkout' ),
			"thumbnail"	 => WPEC_PLUGIN_URL . "/admin/includes/images/wpec-simple-membership-integration.png",
			"description"	 => esc_html__( "Integrate with the Simple Membership plugin to accept membership payments.", 'wp-express-checkout' ),
		...
		$addon_6 = array(
			"name"		 => esc_html__( "MailChimp Integration", 'wp-express-checkout' ),
			"thumbnail"	 => WPEC_PLUGIN_URL . "/admin/includes/images/wpec-mailchimp.png",
			"description"	 => esc_html__( "Automatically add customers to your MailChimp list after a purchase.", 'wp-express-checkout' ),
		...
		foreach ( $addons_data as $addon ) {
			...
			$output	 .= '<img src="' . esc_url( $img_src ) . '" alt="' . esc_attr( $addon[ 'name' ] ) . '">';
			$output	 .= '</div>'; //end thumbnail
	
			$output	 .= '<div class="wpec_addon_item_body">';
			$output	 .= '<div class="wpec_addon_item_name">';
			$output	 .= '<a href="' . esc_url( $addon[ 'page_url' ] ) . '" target="_blank">' . esc_html( $addon[ 'name' ] ) . '</a>';
			$output	 .= '</div>'; //end name
	
			$output	 .= '<div class="wpec_addon_item_description">';
			$output	 .= esc_html( $addon[ 'description' ] );
			$output	 .= '</div>'; //end description
	
			$output	 .= '<div class="wpec_addon_item_details_link">';
			$output	 .= '<a href="' . esc_url( $addon[ 'page_url' ] ) . '" class="wpec_addon_view_details" target="_blank">' . esc_html__( 'View Details', 'wp-express-checkout' ) . '</a>';
