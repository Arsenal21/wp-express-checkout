...
namespace TTHQ\WPEC\Lib\PayPal\Onboarding;

use TTHQ\WPEC\Lib\PayPal\PayPal_Utility_Functions;
use TTHQ\WPEC\Lib\PayPal\PayPal_Request_API;
use TTHQ\WPEC\Lib\PayPal\PayPal_Bearer;
use TTHQ\WPEC\Lib\PayPal\PayPal_PPCP_Config;
...
class PayPal_PPCP_Onboarding_Serverside {
	...
	public function handle_onboarded_callback_data(){
		...
		if ( empty( $data ) ) {
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __( 'Empty data received.', 'wp-express-checkout' ),
				)
			);
		}
		...
		if ( ! check_ajax_referer( $nonce_string, '_wpnonce', false ) ) {
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __( 'Nonce check failed. The page was most likely cached. Please reload the page and try again.', 'wp-express-checkout' ),
				)
			);
			exit;
		}
		...
		if ( ! $access_token ) {
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __( 'Failed to generate access token. check debug log file for any error message.', 'wp-express-checkout' ),
				)
			);
			exit;
		}
		...
		if ( ! $seller_api_credentials ) {
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __( 'Failed to get seller API credentials. check debug log file for any error message.', 'wp-express-checkout' ),
				)
			);
		}
		...
		if( ! $seller_account_status ){
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __( 'Failed to get seller account status. check debug log file for any error message.', 'wp-express-checkout' ),
				)
			);
		}
		...
		if( ! $seller_account_status['payments_receivable'] ){
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __( 'Your PayPal account is limited so you cannot accept payment. Contact PaPal support or check your PayPal account inbox for an email from PayPal for the next steps to remove the account limit.', 'wp-express-checkout' ),
				)
			);
		}
		if( ! $seller_account_status['primary_email_confirmed'] ){
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __( 'Your PayPal account email is not confirmed. Check your PayPal account inbox for an email from PayPal to confirm your PayPal email address.', 'wp-express-checkout' ),
				)
			);
		}
	...
	public function save_seller_api_credentials( $seller_api_credentials, $environment_mode = 'production' ) {
		...
		if( $environment_mode == 'sandbox' ){
			//Sandobx mode
			$settings->set_value('sandbox_client_id', $seller_api_credentials['client_id']);
			$settings->set_value('sandbox_secret_key', $seller_api_credentials['client_secret']);
			$settings->set_value('paypal-sandbox-seller-merchant-id', $seller_api_credentials['payer_id']);//Seller Merchant ID
		} else {
			//Production mode
			$settings->set_value('live_client_id', $seller_api_credentials['client_id']);
			$settings->set_value('live_secret_key', $seller_api_credentials['client_secret']);
	...
	public static function reset_seller_api_credentials( $environment_mode = 'production' ) {
		...
		if( $environment_mode == 'sandbox' ){
			//Sandobx mode
			$settings->set_value('sandbox_client_id', '');
			$settings->set_value('sandbox_secret_key', '');
			...
			//Production mode
			$settings->set_value('live_client_id', '');
			$settings->set_value('live_secret_key', '');
